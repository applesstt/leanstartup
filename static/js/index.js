...
var Index = (function () {
    ...
    var _autoSave = function () {
        var save_dict = {};
        $('.span10').each(function (i, q) {
            var item_index = $(q).parents('.show-grid').attr('id');
            if(!save_dict.hasOwnProperty(item_index)){
                save_dict[item_index] = []
            }
            save_dict[item_index].push($(q).text())
        });

        $.ajax({
            url: '/push/default',
            type: 'post',
            data: {'q': JSON.stringify(save_dict)},
            success: function () {
            },
            error: function () {
                alert('保存失败');
    ...
    var _recover = function () {
        $.ajax({
            ...
            success: function (req) {
                var save = JSON.parse(req);
                var keys = Object.keys(save);
                for(var i in Object.keys(save)){
                    var key = keys[i];
                    var itemList = $('#' + key).find('.item-list');
